...
console.log(`${colors.green}Created backup directory: ${deployBackupDir}${colors.reset}`);
...
filesToBackup.forEach(file => {
  try {
    ...
    if (fs.existsSync(filePath)) {
      ...
      console.log(`${colors.yellow}!${colors.reset} File ${file} not found, skipping backup`);
    }
  } catch (err) {
    console.error(`${colors.red}✗${colors.reset} Error backing up ${file}:`, err.message);
  }
});
...
try {
  ...
  try {
    gitStatus = execSync('git log -1 --oneline', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().trim();
  } catch (e) {
    // Git command failed, use default value
  }
  
  try {
    gitBranch = execSync('git branch --show-current', { stdio: ['pipe', 'pipe', 'ignore'] }).toString().trim();
  } catch (e) {
    // Git command failed, use default value
  }
...
`;
  ...
  console.error(`${colors.red}✗${colors.reset} Error creating deployment log:`, err.message);
}
