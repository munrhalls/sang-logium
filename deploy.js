...
async function deploy() {
  try {
    ...
    if (!backupResult.success) {
      console.error(`${colors.red}Backup failed. Aborting deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!envCheckResult.success) {
      console.error(`${colors.red}Environment verification failed. Aborting deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!testResult.success) {
      console.error(`${colors.red}Tests failed. Aborting deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!lintResult.success) {
      console.error(`${colors.red}Linting failed. Aborting deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!typegenResult.success) {
      console.error(`${colors.red}Type generation failed. Aborting deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    if (!stagingResult.success) {
      console.error(`${colors.red}Staging deployment failed. Aborting production deployment.${colors.reset}`);
      process.exit(1);
    }
    ...
    let previewUrl = '';
    try {
      const match = stagingResult.output.match(/Website Draft URL: (https:\/\/[^\s]+)/);
      if (match && match[1]) {
        previewUrl = match[1];
        console.log(`${colors.green}Preview URL: ${previewUrl}${colors.reset}`);
      }
    } catch (e) {
      console.log(`${colors.yellow}Could not extract preview URL from Netlify output${colors.reset}`);
    }
    ...
    if (answer.toLowerCase() === 'y') {
      ...
      if (!prodResult.success) {
        console.error(`${colors.red}Production deployment failed.${colors.reset}`);
        process.exit(1);
      }
      console.log(`${colors.green}Production deployment successful!${colors.reset}`);
      
      // Step 10: Verify deployment
      printStep('Step 10: Verifying deployment');
      const verifyResult = runCommand('npm run verify');
      if (!verifyResult.success) {
        console.error(`${colors.red}Deployment verification failed. Consider rolling back.${colors.reset}`);
        console.log(`${colors.yellow}Run 'netlify sites:rollback' to rollback to the previous version.${colors.reset}`);
        process.exit(1);
      }
    ...
    console.error(`${colors.red}Unexpected error during deployment:${colors.reset}`, error);
    process.exit(1);
  } finally {
    rl.close();
  }
