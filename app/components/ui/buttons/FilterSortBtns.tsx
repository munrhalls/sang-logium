...
export default function FilterSortBtns() {
  ...
  const hasActiveFilters = Array.from(searchParams.keys()).some(key => 
    !key.includes('sort') && !key.includes('dir')
  );
  
  const hasActiveSort = searchParams.has('sort');
  ...
  return (
    <div className="sticky bottom-0 md:hidden z-40 grid grid-cols-2 gap-2 p-3 bg-white shadow-[0_-2px_10px_rgba(0,0,0,0.1)]">
      ...
      >
        ...
        Filter
        {hasActiveFilters && <span className="ml-1 bg-white text-blue-600 text-xs px-1.5 py-0.5 rounded-full">Active</span>}
      </button>
      ...
      >
        <ArrowUpDown size={18} /> 
        Sort
        {hasActiveSort && <span className="ml-1 bg-white text-blue-600 text-xs px-1.5 py-0.5 rounded-full">Active</span>}
      </button>
    </div>
  );
}
