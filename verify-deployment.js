...
try {
  ...
  console.log(`${colors.yellow}Could not load .env.production file${colors.reset}`);
}
...
async function checkEndpoint(url) {
  try {
    ...
    if (response.ok) {
      console.log(`${colors.green}✓ Success${colors.reset}: ${url} - ${response.status} (${timeMs}ms)`);
      return true;
    } else {
      console.error(`${colors.red}✗ Failed${colors.reset}: ${url} - ${response.status} (${timeMs}ms)`);
      return false;
    }
  } catch (error) {
    console.error(`${colors.red}✗ Error${colors.reset}: ${url} - ${error.message}`);
    return false;
  }
}
...
async function verifyDeployment() {
  ...
  for (const path of ENDPOINTS) {
    try {
      ...
      console.error(`${colors.red}Error checking endpoint ${path}:${colors.reset}`, error.message);
      failedCount++;
    }
  }
  ...
  console.log(`${colors.green}✓ Passed:${colors.reset} ${passedCount}/${ENDPOINTS.length}`);
  
  if (failedCount > 0) {
    console.log(`${colors.red}✗ Failed:${colors.reset} ${failedCount}/${ENDPOINTS.length}`);
    ...
    console.log(`- Run 'netlify sites:rollback' to revert to a previous deployment if needed`);
    process.exit(1);
  } else {
...
verifyDeployment().catch(err => {
  console.error(`${colors.red}Fatal error during verification:${colors.reset}`, err);
  process.exit(1);
});